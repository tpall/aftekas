process VAMB_BIN {
    ...
    output:
    tuple val(meta), path("${prefix}*.fa.gz")                    , emit: bins             , optional: true
    ...
    assert args =~ "--minfasta [0-9]+" : "ERROR: Please supply minimum contig length via --minfasta argument to Vamb!"
    ...
    for file in ${prefix}/bins/*.fna.gz ; do
        [[ \$(basename \${file}) =~ (.*).fna.gz ]];
        bin="\${BASH_REMATCH[1]}"
        mv \${file} ${prefix}.\${bin}.fa.gz
    done
